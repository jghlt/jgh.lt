...
class ScrollSpy extends React.Component<{}, { sections: any, currentSection: any }> {
  ...
  checkSections = () => {
    ...
    const currentSection = this.state.sections.filter((section) => {
      return (
        activePosition > section.bounds.top &&
        activePosition < section.bounds.bottom
      );
    });
    const setCurrentSection =
      currentSection.length &&
      currentSection[0].id !== this.state.currentSection
        ? currentSection[0].id
        : false;
            ...
            {sections.map((section, index) => {
              return (
                <div key={index} className="pa2">
                  <button
                    onClick={(event) => this.scrollToSection(event, index)}
                    className={`_o-dot pointer pa0 b--none ${
